...
describe('EnrollmentsService', () => {
    ...
    describe('enrollInCohort', () => {
        ...
        it('should throw NotFoundException if cohort does not exist', async () => {
            ...
            await expect(service.enrollInCohort(userId, cohortId)).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw ConflictException if already enrolled', async () => {
            ...
            mockPrismaService.enrollment.findUnique.mockResolvedValue({ id: 'enr-1' });

            await expect(service.enrollInCohort(userId, cohortId)).rejects.toThrow(ConflictException);
        });
        ...
        it('should enroll user and return new enrollment record', async () => {
            ...
            const expectedRecord = { id: 'enr-1', user_id: userId, cohort_id: cohortId, status: EnrollmentStatus.ACTIVE, progress_percent: 0 };
    ...
    describe('updateProgress', () => {
        ...
        it('should throw NotFoundException if enrollment invalid or doesnt belong to user', async () => {
            ...
            await expect(service.updateProgress(userId, enrollmentId, newProgress)).rejects.toThrow(NotFoundException);
        });

        it('should update and return enrollment progress', async () => {
            mockPrismaService.enrollment.findFirst.mockResolvedValue({ id: enrollmentId, user_id: userId, progress_percent: 0 });
            mockPrismaService.enrollment.update.mockResolvedValue({ id: enrollmentId, progress_percent: newProgress });

            const result = await service.updateProgress(userId, enrollmentId, newProgress);
    ...
    describe('updateStatus', () => {
        ...
        it('should throw NotFoundException if enrollment does not exist', async () => {
            ...
            await expect(service.updateStatus(adminId, enrollmentId, newStatus)).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw ForbiddenException if requester is not an org facilitator/admin', async () => {
            ...
            await expect(service.updateStatus(adminId, enrollmentId, newStatus)).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should update status when authorized', async () => {
            ...
            mockPrismaService.orgMember.findUnique.mockResolvedValue({ role: OrgRole.FACILITATOR });
            mockPrismaService.enrollment.update.mockResolvedValue({ id: enrollmentId, status: newStatus });

            const result = await service.updateStatus(adminId, enrollmentId, newStatus);
