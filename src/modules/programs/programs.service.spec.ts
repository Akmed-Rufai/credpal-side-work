...
describe('ProgramsService', () => {
    ...
    describe('createProgram', () => {
        ...
        it('should throw ForbiddenException if user is not a member of the organization', async () => {
            ...
            await expect(service.createProgram(userId, orgId, dto)).rejects.toThrow(ForbiddenException);
        });

        it('should create a program if user is a valid member', async () => {
            mockPrismaService.orgMember.findUnique.mockResolvedValue({ role: 'ADMIN' });
            const expectedOutput = { id: 'prog-1', org_id: orgId, ...dto, status: 'DRAFT' };
    ...
    describe('createCohort', () => {
        ...
        const orgId = 'org-1';
        const dto = { name: 'Cohort 1', startDate: '2023-10-01T00:00:00Z', endDate: '2023-12-01T00:00:00Z' };
        ...
        it('should throw NotFoundException if program does not exist', async () => {
            ...
            await expect(service.createCohort(userId, programId, dto)).rejects.toThrow(NotFoundException);
        });

        it('should throw ForbiddenException if user is not part of the org owning the program', async () => {
            mockPrismaService.program.findUnique.mockResolvedValue({ id: programId, org_id: orgId });
            mockPrismaService.orgMember.findUnique.mockResolvedValue(null);

            await expect(service.createCohort(userId, programId, dto)).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should create a cohort on valid access', async () => {
            ...
            const expectedCohort = { id: 'coh-1', program_id: programId, name: dto.name };
