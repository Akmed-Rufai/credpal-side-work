...
describe('WebhooksService', () => {
  ...
  describe('handlePaystackWebhook', () => {
    ...
    const validSignature = crypto
      .createHmac('sha512', 'mocked-secret')
      .update(rawBody)
      .digest('hex');

    it('should throw BadRequestException on invalid signature', async () => {
      await expect(
        service.handlePaystackWebhook('invalid-sig', rawBody, payload),
    ...
    it('should handle charge.success via transaction update and enrollment', async () => {
      ...
      expect(mockEnrollmentsService.enrollInCohort).toHaveBeenCalledWith(
        'user-1',
        'cohort-1',
      );
