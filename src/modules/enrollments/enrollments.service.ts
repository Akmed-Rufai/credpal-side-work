...
export class EnrollmentsService {
    ...
    async updateStatus(adminId: string, enrollmentId: string, status: EnrollmentStatus) {
        ...
        if (!membership || membership.role === OrgRole.LEARNER) {
            throw new ForbiddenException('You do not have permission to modify enrollment status for this organization');
        }
        ...
        return this.prisma.enrollment.findMany({
            ...
            include: {
                cohort: {
                    include: {
                        program: {
                            select: { title: true, cover_image: true, org: { select: { name: true } } },
                        },
                    },
                },
            },
