...
import { CreateOrganizationDto, UpdateOrganizationDto, AddMemberDto } from './dto/organizations.dto';
...
export class OrganizationsService {
    ...
    async createOrganization(userId: string, data: CreateOrganizationDto) {
        ...
        if (existing) throw new ConflictException('Organization alias/slug already taken');
    ...
    async getOrganizationById(orgId: string, userId: string) {
        ...
        if (!org) throw new NotFoundException('Organization not found or access denied');
        return org;
    }
    ...
    async addMember(orgId: string, adminId: string, data: AddMemberDto) {
        ...
        const targetUser = await this.prisma.user.findUnique({ where: { email: data.email } });
        if (!targetUser) {
            throw new NotFoundException('User with this email not found in the platform');
        }
